...
export default function DischargeScreen() {
  ...
  const handleSave = async () => {
    if (!isValidDateString(enlistDate)) {
      Alert.alert('오류', 'YYYY-MM-DD 형식으로 입대일을 입력해주세요.\n예) 2024-02-22');
      return;
    }
    ...
    const mi = { enlistDate, branch, dischargeDate: formatDate(dischargeDate), months: selectedBranch.months };
    ...
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <Card>
          ...
          <TextInput
            style={[styles.input, saved && styles.inputDisabled]}
            value={enlistDate}
            onChangeText={setEnlistDate}
            placeholder="YYYY-MM-DD (예: 2024-02-22)"
          ...
          <Text style={[styles.label, { marginTop: 18 }]}>군별 선택</Text>
          <View style={styles.branchRow}>
            {BRANCHES.map((b) => (
              <TouchableOpacity
                key={b.key}
                style={[styles.branchBtn, branch === b.key && styles.branchBtnActive, saved && styles.branchBtnDisabled]}
                onPress={() => !saved && setBranch(b.key)}
                disabled={saved}
              >
                <Text style={styles.branchEmoji}>{b.emoji}</Text>
                <Text style={[styles.branchLabel, branch === b.key && styles.branchLabelActive]}>{b.label}</Text>
                <Text style={[styles.branchMonths, branch === b.key && styles.branchMonthsActive]}>{b.months}개월</Text>
              </TouchableOpacity>
            ))}
          </View>
            ...
            <Card style={styles.resultCard}>
              ...
              <View style={styles.resultRow}>
                ...
                <View style={styles.resultItem}>
                  ...
                  <Text style={[styles.resultValue, { color: COLORS.accent }]}>{formatDateKo(info.dischargeDate)}</Text>
                </View>
              </View>
...
const styles = StyleSheet.create({
  ...
  ddayBox: {
    ...
    backgroundColor: COLORS.primary, borderRadius: 12, marginBottom: 20,
  },
